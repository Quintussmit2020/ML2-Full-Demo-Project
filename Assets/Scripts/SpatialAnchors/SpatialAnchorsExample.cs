...
public class SpatialAnchorsExample : MonoBehaviour
{
    ...
    private void MenuStarted(InputAction.CallbackContext obj)
    {
       //Get the pose of the controller to set the pose of the created object
        ...
        if (result.IsOk)
        {
            ...
            savedAnchor.Bind(anchor, stickyTextToSave, Prefab2.name, Prefab2.name); //Need to check what this does exactly
    ...
    private void TriggerStarted(InputAction.CallbackContext obj)
    {
        ...
        {
            SimpleAnchorBinding savedAnchor = new SimpleAnchorBinding();
            savedAnchor.Bind(anchor, stickyTextToSave, Prefab1.name, Prefab1.name);
